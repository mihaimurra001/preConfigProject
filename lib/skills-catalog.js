...
import { ALL_100_DESIGN_THEMES, buildDesignPresetsMap } from "./themes-catalog.js";
...
4. **Prevent**: Leave a regression test in the test suite.`
  ...
  {
    id: "database-and-migrations-architect",
    name: "database-and-migrations-architect",
    title: "Database & Zero-Downtime Migrations",
    command: "/db-migrate",
    category: "build",
    description: "Expand-and-contract pattern, safe zero-downtime schema migrations, composite index optimization, and N+1 query prevention.",
    content: `---
name: database-and-migrations-architect
...
- Profile query plans using \`EXPLAIN (ANALYZE, BUFFERS)\` before committing complex queries.`
  ...
  {
    id: "root-cause-analysis-and-postmortem",
    name: "root-cause-analysis-and-postmortem",
    title: "Root Cause Analysis (5 Whys & Regression Reproduction)",
    command: "/rca",
    category: "verify",
    description: "5 Whys analysis, symptom vs root cause dissection, caller grep verification, and mandatory reproduction tests.",
    content: `---
name: root-cause-analysis-and-postmortem
...
- Fix the shared contract once; never duplicate defensive checks across ten different caller files.`
  ...
  {
    id: "api-contracts-and-backward-compat",
    name: "api-contracts-and-backward-compat",
    title: "API Contracts & Backward Compatibility",
    command: "/api-contract",
    category: "build",
    description: "RFC 7807 problem details, strict schema validation (Zod/Pydantic), idempotency keys, and semver evolution.",
    content: `---
name: api-contracts-and-backward-compat
...
- Mark deprecated fields with \`@deprecated\` and log deprecation telemetry before removal.`
  },
  {
    ...
    title: "Concurrency, Race Conditions & State Integrity",
    command: "/concurrency",
    category: "verify",
    description: "Atomic DB updates, TOCTOU prevention, distributed lock safety, and thread-safe async invariants.",
    content: `---
name: concurrency-and-race-conditions
...
- Verify lock ownership token before releasing to prevent releasing another worker's acquired lock after a TTL timeout.`
  ...
  {
    id: "observability-and-structured-logging",
    name: "observability-and-structured-logging",
    title: "Observability, Structured Logging & Health",
    command: "/telemetry",
    category: "ship",
    description: "Structured JSON logs, OpenTelemetry trace context, request ID propagation, and /healthz endpoints.",
    content: `---
name: observability-and-structured-logging
...
- Implement standardized health endpoints:
  ...
  },
  {
    ...
    command: "/adr",
    category: "plan",
    description: "Standardized technical decision logging in docs/adr/ capturing context, decision, trade-offs, and rejected options.",
    content: `---
name: architecture-decision-records
...
\`\`\``
  },
  {
    ...
    title: "Accessibility (WCAG 2.2 AA & ARIA)",
    command: "/a11y",
    category: "verify",
    description: "WCAG 2.2 AA compliance, semantic HTML, keyboard navigation, modal focus traps, and screen reader labels.",
    content: `---
name: accessibility-and-inclusive-design
...
- Ensure form inputs have associated \`<label for="...">\` tags.`
  },
  {
    ...
    title: "Legacy Code Refactoring (Strangler Fig)",
    command: "/refactor-legacy",
    category: "build",
    description: "Characterization tests, golden master snapshots, Strangler Fig pattern, and step-by-step decoupling without rewrites.",
    content: `---
name: refactoring-legacy-code
...
const HANDCRAFTED_DESIGN_MD_PRESETS = {
  linear: `---
version: 1.0
...
export const DESIGN_MD_PRESETS = buildDesignPresetsMap(HANDCRAFTED_DESIGN_MD_PRESETS);

export const AWESOME_DESIGN_THEMES = ALL_100_DESIGN_THEMES;
